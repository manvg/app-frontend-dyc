...
describe('SolicitudProductoComponent', () => {
  ...
  it('debe enviar la solicitud con éxito', fakeAsync(() => {
    ...
    tick(); // procesa la suscripción al observable
    ...
    // <<<<--- AVANZA EL TIEMPO PARA EL setTimeout DEL COMPONENTE
    tick(4000); // ¡pon el mismo valor que el setTimeout real!
    // O simplemente flush();

    // Si quieres, ahora puedes esperar que enviado sea false
    expect(component.enviado).toBeFalse();

    flush(); // Limpia cualquier timer extra
  }));
